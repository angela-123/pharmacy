...
$wela = "select username,password,page from users where username = '$user'";
	...
	{   print '<div id = "jim">';
        ...
        ?>
        <form action="yrevs.php" method="post" id="mini">
        ...
        $sqlx = "select monthname(ddate) as date, sum(qtysold * unitprice -qtysold * unitcost) as profit from dailies where  year(ddate)= '$yr' group by monthname(ddate)";
        $sqlxx = "select sum(qtysold * unitprice -qtysold * unitcost) as profit from dailies where  year(ddate)= '$yr'";
        ...
        $rez = mysql_query($sqlx);
         ...
         $res = mysql_query($sqlx);
                ...
                echo '<table id = diaga title = "YEARLY PROFIT">';
